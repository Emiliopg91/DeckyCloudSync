...
class Deployer:
    ...
    def _clearing_folder(self):
        ...
        Utils.run_command([
            ...
            "-i", Utils.id_rsa_file,
            f"mkdir {self.deck_dir}/homebrew/plugins/{self.plugin_name}/"
    ...
    def _chmod_folders(self):
        ...
        Utils.run_command([
            "ssh", f"{self.deck_user}@{self.deck_ip}", "-p", str(self.deck_port),
            "-i", Utils.id_rsa_file,
            f"echo '{self.deck_pass}' | sudo -S chmod -R 777 {self.deck_dir}/homebrew/plugins"
    ...
    def _restart_decky(self):
        ...
        Utils.run_command([
            "ssh", f"{self.deck_user}@{self.deck_ip}", "-p", str(self.deck_port),
            "-i", Utils.id_rsa_file,
            f"echo '{self.deck_pass}' | sudo -S systemctl restart plugin_loader.service"
            ], False, self.log_restart_decky)

    def deploy(self):
        print(f"Deploying plugin {self.plugin_name}")
