...
class Builder:
    def __init__(self):
        ...
        self.plugin_backend_dockerfile_dir = os.path.join(
            self.plugin_backend_dir, "Dockerfile"
        )
        ...
        self.log_backend_build_backend = os.path.join(
            self.log_dir, "03-backend-build.log"
        )
        self.log_copy_additional_files = os.path.join(
            self.log_dir, "04-copy-additional-files.log"
        )
    ...
    def _clean_directories(self):
        ...
        if os.path.exists(self.build_out_base_dir) or os.path.exists(
            self.build_dist_dir
        ):
            if os.path.exists(self.build_dist_dir):
                Utils.run_command(
                    ["rm", "-r", self.build_dist_dir], True, self.log_clean_workspace
                )
    ...
    def _build_frontend(self):
        ...
        Utils.run_command(
            ["pnpm", "install", "--no-frozen-lockfile"], True, self.log_build_frontend
        )
        Utils.run_command(["pnpm", "run", "build"], True, self.log_build_frontend)
        Utils.run_command(
            ["mkdir", "-p", self.build_out_dist_dir], False, self.log_build_frontend
        )
        Utils.run_command(
    ...
    def _build_backend(self):
        ...
        if os.path.exists(self.plugin_backend_dir) and os.path.exists(
        ...
        with open(self.log_binary_copy, "a") as log_file:
            ...
            for binary in remote_binaries:
                ...
                if not url or not expected_checksum or not dest_filename:
                    log_file.write(
                        "      Error: Missing fields in devRemoteBinaries configuration\n"
                    )
                    exit(1)
                ...
                if os.path.exists(dest_file_path):
                    actual_checksum = hashlib.sha256(
                        open(dest_file_path, "rb").read()
                    ).hexdigest()
                    if actual_checksum == expected_checksum:
                        print(
                            "      File already exists and checksums match: "
                            + dest_file_path
                        )
                        continue
                    else:
                        print(
                            "      Checksum mismatch for existing file, removing: "
                            + dest_file_path
                        )
